...
function Start-Validation {
  try {
    ...
    $psi.Arguments = ('"{0}" --input "{1}" --instance "{2}" --output "{3}" --db-connections "{4}" --sqlplus "{5}"' -f `
      $scriptPath, $excelPath, $instance, $outputPath, $script:dbConnectionsPath, $script:sqlplusPath)

    $proc = New-Object System.Diagnostics.Process
    $proc.StartInfo = $psi
    $proc.EnableRaisingEvents = $false

    $script:outputHandler = [System.Diagnostics.DataReceivedEventHandler]{
      param($sender, $eventArgs)
      if (-not [string]::IsNullOrWhiteSpace($eventArgs.Data)) {
        Enqueue-Log $eventArgs.Data
      }
    }

    $script:errorHandler = [System.Diagnostics.DataReceivedEventHandler]{
      param($sender, $eventArgs)
      if (-not [string]::IsNullOrWhiteSpace($eventArgs.Data)) {
        Enqueue-Log ("ERROR: " + $eventArgs.Data)
      }
    }

    $proc.add_OutputDataReceived($script:outputHandler)
    $proc.add_ErrorDataReceived($script:errorHandler)

    $started = $proc.Start()
    if (-not $started) {
      throw "No se pudo iniciar node.exe"
    }
    ...
    Add-Log ("ERROR al iniciar la validación: " + $_.Exception.Message)
...
function Stop-Validation {
  ...
  if ($script:outputHandler) {
    try { $script:currentProcess.remove_OutputDataReceived($script:outputHandler) } catch { }
  }
  if ($script:errorHandler) {
    try { $script:currentProcess.remove_ErrorDataReceived($script:errorHandler) } catch { }
  }
  $script:outputHandler = $null
  $script:errorHandler = $null
  $script:currentProcess = $null
...
$script:form.Text = "Validador de Cédulas B2000"
...
$lblTitle.Text = "Validador de Cédulas por Instancia Oracle"
...
[System.Windows.Forms.Application]::add_ThreadException({
  ...
  Invoke-SafeUi -Context "ThreadException" -Action {
    $msg = if ($eventArgs -and $eventArgs.Exception) { $eventArgs.Exception.Message } else { "Excepción no controlada en UI." }
...
[System.AppDomain]::CurrentDomain.add_UnhandledException({
  ...
  Invoke-SafeUi -Context "UnhandledException" -Action {
    $msg = if ($eventArgs -and $eventArgs.ExceptionObject) { [string]$eventArgs.ExceptionObject } else { "Excepción no controlada en AppDomain." }
